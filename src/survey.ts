...
const SUBMIT_KEY         = 'hmg-survey-submitted'  // localStorage 키
...
const questions: Question[] = [
  ...
  },
  {
    id: 'ai_experience',
    type: 'radio',
    title: 'AI 관련 업무 경험이 있으신가요?',
    required: true,
    options: [
      '없음 (처음 접함)',
      '간접 경험 (ChatGPT 등 개인적으로 사용)',
      '업무 활용 경험 있음',
      'AI 프로젝트 참여 경험 있음',
    ],
  },
  {
    id: 'expectation',
    type: 'checkbox',
    title: '이번 과정에서 가장 기대하는 것을 모두 선택해 주세요.',
    required: false,
    options: [
      'AI 개념 및 트렌드 이해',
      'AI 솔루션 기획 방법론 습득',
  ...
  },
  {
    id: 'ai_tool_level',
    type: 'rating',
    title: '현재 AI 도구(ChatGPT, Copilot 등) 활용 수준을 평가해 주세요.',
    description: '1 = 전혀 사용 안 함 · 5 = 일상 업무에 적극 활용',
    required: true,
    min: 1,
    max: 5,
  },
  {
    id: 'requests',
    type: 'textarea',
    title: '강사/운영진에게 전달하고 싶은 요청 사항이나 궁금한 점을 자유롭게 적어 주세요.',
    required: false,
  },
]
    ...
    .map((q, i) => {
      ...
      return `
        <div class="question-block" id="block-${q.id}">
          <p class="question-title">${num} ${q.title}${required}</p>
          ${desc}
          ${input}
...
function renderInput(q: Question): string {
  switch (q.type) {
    ...
    case 'select':
      return `
        <select class="form-select" id="q-${q.id}" name="${q.id}">
      ...
      return `
        <div class="rating-group">
          ${labels
            ...
            <label class="rating-label" for="r-${q.id}-${v}">${v}</label>`
            )
            .join('')}
        </div>`
    }
...
async function handleSubmit(e: Event) {
  ...
  try {
    ...
    console.error(err)
    if (banner) {
      banner.textContent = '제출 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.'
      banner.hidden = false
    }
